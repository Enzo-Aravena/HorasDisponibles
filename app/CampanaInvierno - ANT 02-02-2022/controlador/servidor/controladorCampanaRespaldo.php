...
switch ($evento) {
	...
	case 'cargarTabla':
		...
		if($semana == 0 ){
	 		if ($centro ==0) {
	 			$data = $modelo->CAMIN_OBTENER_DATOS_TABLA_UNO($conn);
	 			//echo "MUESTRA EL PRIMERO";

	 			//print_r($data);
	 		}else{
	 			$centro = $_GET['centro'];
	 			$modelo->setCentro($centro);
	 			$data = $modelo->CAMIN_OBTENER_DATOS_TABLA_DOS($conn);
	 			//echo "MUESTRA EL DOS";
	 		}
	 	}else{
	 		if ($centro ==0) {
	 			$modelo->setSemana($semana);
	 			$data = $modelo->CAMIN_OBTENER_DATOS_TABLA_TRES($conn);	 
	 			//echo "MUESTRA EL TRES";			
	 		}else{
				$modelo->setSemana($semana);
				$modelo->setCentro($centro);
	 			$data = $modelo->CAMIN_OBTENER_DATOS_TABLA_CUATRO($conn);
	 			//echo "MUESTRA EL CUATRO";
	 		}
	 	}
			...
			echo "<tr>";
			if($semana == 0){
			echo "<th>ATENCIONES TOTAL SEMANAS EPIDEMIOLOGICAS 2019</th>";
			}else{echo "<th>ATENCIONES SEMANAS EPIDEMIOLOGICAS $semana 2019</th>";}
