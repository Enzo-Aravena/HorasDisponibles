...
$( document ).ready(function() {
  ...
  $('#ejecutar').on("click",function(){
    ...
    if (porTagName === "") {
      $("#mensaje").empty();
      texto = "<h3> No ha seleccionado un archivo. </h3>";
      $("#mensaje").append(texto);
      $("#myModal").modal({backdrop: 'static', keyboard: false});
    }else{
        if (ValidaArchivo(porTagName) === false) {
          $("#mensaje").empty();
          texto = "<h3> La extension no es valida. </h3>";
          $("#mensaje").append(texto);
          $("#myModal").modal({backdrop: 'static', keyboard: false});
              ...
              resultado = response;
                switch(resultado){
                  case "1":
                    ...
                    $("#close").hide();
                    $("#cerrar").hide();
                    texto = "<h3> Archivo Cargado Exitosamente!......  </h3>";
                    $("#mensaje").append(texto);
                    $("#myModal").modal({backdrop: 'static', keyboard: false});
                  ...
                  case "2":
                    ...
                    $("#close").show();
                    $("#cerrar").show();
                    texto = "<h3> Error al cargar al archivo. </h3>";
                    $("#mensaje").append(texto);
                    $("#myModal").modal({backdrop: 'static', keyboard: false});
                  break;
                  ...
                  case "4":
                    ...
                    $("#close").show();
                    $("#cerrar").show();
                    texto = "<h3> El archivo ya fue cargado. </h3>";
                    $("#mensaje").append(texto);
                    $("#myModal").modal({backdrop: 'static', keyboard: false});
                  break;
                  ...
                  case "5":
                    ...
                    $("#close").show();
                    $("#cerrar").show();
                    texto = texto + "Error al cargar el archivo, Las columnas no corresponden a la estructura asignada. Revise si hay espacios en la estructura enviada (Ver ejemplo en i )";
                    $("#mensaje").append(texto);
                    $("#myModal").modal({backdrop: 'static', keyboard: false});
                  break;
                  ...
                  default:
                    ...
                    for(aux in datos.filas){
                      var tabla = "";
                      if (datos.filas[aux].col2 === "-" && datos.filas[aux].col3 === "-" && datos.filas[aux].col4 === "-" && datos.filas[aux].col5 === "-" && datos.filas[aux].col6 === "-" && datos.filas[aux].col7 === "-" ){
                      }else{
                        tabla = tabla + '<tr>';
                          ...
                          if (datos.filas[aux].col7 === "-") {  
                            tabla = tabla + '<td  class="text-center">'+datos.filas[aux].col7+'</td>';
                          }else{
                            if (datos.filas[aux].col7 === null) {
                              tabla = tabla + '<td  class="text-center" style = "color:red;"> Campo vacio </td>';  
                            } else {
                              tabla = tabla + '<td  class="text-center" style = "color:red;">'+datos.filas[aux].col7+'</td>';
                            }
