...
class claseCampana
{
	...
	public function OBTENER_SEMANAS_EPIDEMIOLOGICA($bd){
		$i = 0;
		$data = Array();
		$sql= "call OBTENER_SEMANAS_EPIDEMIOLOGICA(2021)";
	...
	public function OBTENER_SEMANA_EPIDEMIOLOGICA($bd){
		...
		SE.SEMANA_EPI IN ({$this->getSemana()}) and ANNO = 2021 ORDER BY SE.FECHA ASC LIMIT 1) as 
		'FECHA',(SELECT DATE_FORMAT(SE.FECHA_FINAL_EPI, '%d-%m-%Y')
		FROM semana_epidemiologica SE where SE.SEMANA_EPI IN ({$this->getSemana()}) and ANNO = 2021 
