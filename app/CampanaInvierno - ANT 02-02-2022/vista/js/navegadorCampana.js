$( document ).ready(function() {
	...
	$("#ExportarData").on("click",function(){
		...
		if(sem == null){	var semana = 0;	}
		else{	var semana = $('#semana').val().toString();	}
		setTimeout(cuentaAtras, 1000);
...
function cuentaAtras() {
  if (contador==0) {	$("#myModal").modal('hide');	} else {	contador--;	setTimeout(cuentaAtras, 2000);  }
}
//------------------------------------------
	...
	var data= {
		...
		$.ajax({
			...
			success:function(response){	
				...
				$('#semana').empty();
				$("#semana").append('<option value=0 selected="selected">______Total Acumulado 2021______</option>');
...
function enviarDatosFormulario(){
	...
	if(dia !== undefined){	dia = 1;	}else{	dia = 0;	}
	var centro = ($("input[name='centro']:checked").val());
	var aux_centro =  obtener_centro($("input[name='centro']:checked").val());
	var sem = $('select[name=semana]').val();
	if(sem == null){	var semana = 0;	}	else{	var semana = $('#semana').val().toString();	}
	var Semanas= $('select[name=semana]').val();
	 if (Semanas[0] === "0") {
		$("#visualizador").hide();
		$("input[name='visualizar_dia']:checked").attr('checked', false);
		var dia = $("input[name='visualizar_dia']:checked").val();
		if(dia !== undefined){	dia = 1;}else{	dia = 0;}
	}
	else{
	...
	$.getJSON("../controlador/servidor/controladorCampana.php",data, function(json) {
		...
		$.getJSON("../controlador/servidor/controladorCampana.php",dato, function(json) {
			...
			if(semana === "0" || semana === 0){ 
				...
				options.subtitle.text ='Semanas Epidemiologica Entre '+FECHA+" al "+FECHA_FINAL_EPI+'  -> '+ aux_centro;
			}
   	...
   	var options = {
        ...
        plotOptions: {// quita el texto dentro del grafico
	        series: {
	            label: {
	                enabled: false
	            }
		...
		series: [		
			{ 
				id :2,		
				name: 'Total Atenciones de Morbilidad (contiene las respiratorias) 2020',
			...
			{
			 	id :1,	
				name: 'Total Atenciones de Morbilidad (contiene las respiratorias) 2021',
	                    data: V_TOTAL_CESFAM,
						color: {
	                linearGradient : {
	                    x1: 0,
			...
			{ 
				id :2,
				name: 'Total Atenciones por causa Sistema Respiratorio 2020',
				data: V_TOTAL_SR_AA,
				color: {
					linearGradient : {
						x1: 0,
        	...
        	{
				id :1,
				name: 'Total Atenciones por causa Sistema Respiratorio 2021',
				data: V_TOTAL_SR,
				color: {
					linearGradient : {
						x1: 0,
